...
def nw(x, y, match = 1, mismatch = 1, gap = 1):
    '''
    Needleman-Wunsch Algorithm obtained from https://gist.github.com/slowkow/06c6dba9180d013dfd82bec217d22eb5
    '''
    nx = len(x)
    ny = len(y)
    ...
    rx = ''.join(rx)[::-1]
    ry = ''.join(ry)[::-1]
    # Different return from source, only giving scores.
    total_mm = sum(c1!=c2 for c1,c2 in zip(rx,ry))
    indels = rx.count("-") + ry.count("-")
